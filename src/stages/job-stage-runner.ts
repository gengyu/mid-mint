...
export class JobStageRunner {
  constructor(
    protected readonly repositories: WorkflowRepositories,
    protected readonly modules: WorkflowModules
  ) {}
  ...
  async finalizeReview(jobId: string, versionNumber: number) {
    ...
    if (reviewResult.decision === "approve") {
      ...
      return { status: "APPROVED" as const, errorCode: null };
    }

    if (reviewResult.decision === "block") {
      this.updateJob(jobId, (job) => ({ ...job, status: "FAILED" }));
      return { status: "FAILED" as const, errorCode: "REVIEW_BLOCKED" };
    }
  ...
  protected updateJob(jobId: string, updater: (job: Job) => Job): Job {
